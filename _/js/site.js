;(function () {
  ...
  styleSignupClassResponsive()
  window.onresize = styleSignupClassResponsive
  ...
  function styleSignupClassResponsive () {
    var signupElement = document.querySelector('#signup')

    if (window.innerWidth >= 1024) {
      if (!signupElement.classList.contains('button')) {
        signupElement.classList.add('button')
      }
    } else {
      if (signupElement.classList.contains('button')) {
        signupElement.classList.remove('button')
      }
    }
  }
})()
...
// Disable some eslint checks so that sample countly code works:
